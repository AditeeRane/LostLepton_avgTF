...
void Prediction::SlaveBegin(TTree * /*tree*/)
{
  ...
  h_SearchStat = new TH1D("h_SearchStat","h_SearchStat",nSB, 0.5, nSB+0.5);

  h_HT_SearchStat = new TH1D("h_HT_SearchStat","h_HT_SearchStat",50, 300, 10300);
  h_MHT_SearchStat = new TH1D("h_MHT_SearchStat","h_MHT_SearchStat",60, 300, 3300);
  h_NJet_SearchStat = new TH1D("h_NJet_SearchStat","h_NJet_SearchStat",20,0,20);
  h_NBTag_SearchStat = new TH1D("h_NBTag_SearchStat","h_NBTag_SearchStat",10,0,10);

  double binHT[]={300.,500.,1000.,9999.};
  double binMHT[]={300.,350.,500.,750.,9999.};
  double binNJ[]={2,3,5,7,9,20};
  double binNbJ[]={0,1,2,3,20};
  int NbinHT= sizeof(binHT)/sizeof(double)-1;
  int NbinMHT= sizeof(binMHT)/sizeof(double)-1;
  int NbinNJ= sizeof(binNJ)/sizeof(double)-1;
  int NbinNbJ=sizeof(binNbJ)/sizeof(double)-1;

  std::cout<<" NbinHT "<<NbinHT<<endl;
  std::cout<<" NbinMHT "<<NbinMHT<<endl;
  std::cout<<" NbinNJ "<<NbinNJ<<endl;
  std::cout<<" NbinNbJ "<<NbinNbJ<<endl;

  h_HTBins = new TH1D("h_HTBins","h_HTBins",NbinHT,binHT);
  h_MHTBins = new TH1D("h_MHTBins","h_MHTBins",NbinMHT,binMHT);
  h_NJetBins = new TH1D("h_NJetBins","h_NJetBins",NbinNJ,binNJ);
  h_NBTagBins = new TH1D("h_NBTagBins","h_NBTagBins",NbinNbJ,binNbJ);

  std::cout<<" bins in h_HTBins "<<h_HTBins->GetNbinsX()<<endl;

  GetOutputList()->Add(h_Prediction);
  GetOutputList()->Add(h_CSStat);
  GetOutputList()->Add(h_SearchStat);
  GetOutputList()->Add(h_HT_SearchStat);
  GetOutputList()->Add(h_MHT_SearchStat);
  GetOutputList()->Add(h_NJet_SearchStat);
  GetOutputList()->Add(h_NBTag_SearchStat);
  GetOutputList()->Add(h_HTBins);
  GetOutputList()->Add(h_MHTBins);
  GetOutputList()->Add(h_NJetBins);
  GetOutputList()->Add(h_NBTagBins);
...
*/
  ...
  if(ElectronsNum_ + MuonsNum_ == 0 && isoTracksNum == 0){
    ...
    std::cout<<" entry "<<entry<<" HT "<<HT<<" MHT "<<MHT<<" NJets "<<NJets<<endl; 
   if(useDeltaPhiCut == 1) if(DeltaPhi1 < deltaPhi1_ || DeltaPhi2 < deltaPhi2_ || DeltaPhi3 < deltaPhi3_ || DeltaPhi4 < deltaPhi4_) return kTRUE;
    ...
    if(runOnSignalMC){ //false for SM MC
      ...
      std::cout<<" currentTree "<<currentTree<<" entry "<<entry<<endl;
    ...
    if(runOnSignalMC){
      ...
      std::cout<<" entry "<<entry<<" weight_xsec "<<Weight<<endl;
    ...
    for(int i = 0; i < nLoops; i++){
      ...
      std::cout<<" entry "<<entry<<" bin "<<bTagBin<<" weight_btagProb "<<WeightBtagProb<<endl;
      h_SearchStat->Fill(bTagBin, WeightBtagProb);
      h_HT_SearchStat->Fill(HT, WeightBtagProb);
      h_MHT_SearchStat->Fill(MHT, WeightBtagProb);
      h_NJet_SearchStat->Fill(NJets, WeightBtagProb);
      h_NBTag_SearchStat->Fill(BTags, WeightBtagProb);
      h_HTBins->Fill(HT, WeightBtagProb);
      h_MHTBins->Fill(MHT, WeightBtagProb);
      h_NJetBins->Fill(NJets, WeightBtagProb);
      h_NBTagBins->Fill(BTags, WeightBtagProb);
    }
  }
  return kTRUE;
}
...
void Prediction::Terminate()
{
  ...
  h_SearchStat = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_SearchStat"));
  h_HT_SearchStat = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_HT_SearchStat"));
  h_MHT_SearchStat = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_MHT_SearchStat"));
  h_NJet_SearchStat = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_NJet_SearchStat"));
  h_NBTag_SearchStat = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_NBTag_SearchStat"));
  h_HTBins = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_HTBins"));
  h_MHTBins = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_MHTBins"));
  h_NJetBins = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_NJetBins"));
  h_NBTagBins = dynamic_cast<TH1D*>(GetOutputList()->FindObject("h_NBTagBins"));
