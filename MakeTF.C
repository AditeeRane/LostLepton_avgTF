...
void MakeTF(const char *InputTxtFile,const char * subSampleKey, const char * Outdir, const char * inputnumber,const char * verbosity)
{
...
*/
  ...
  for(Int_t i=0; i<nChains; i++){ //i<nChains i>2
    ...
    Effchain[i]->Process("TFMaker", TString::Format("%s/TF_%d_%s_%s.root",Outdir,i,subSampleKey,inputnumber));
