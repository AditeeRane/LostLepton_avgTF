...
void Plot_searchBin_comparison(string option="", int pull=0){ // string option="QCD"
  ...
  sprintf(tempname,"Prediction_0_haddTTbarWJetST_Jul02Exp_2016_hadd.root");
  ...
  sprintf(tempnameAvg,"Prediction_0_haddTTbarWJetST_Jul02Pre_2016_hadd.root");
 ...
 // false: do closure test (MC prediction vs MC truth)
  ...
  bool NjNbcorr=false; //true only for MC prediction from Moriond
  ...
  //*AR: 190112: Read MC exp file
  TFile * LLFile = new TFile(tempname,"R"); //MC exp/pred from Moriond
  printf("Opened %s\n",tempname);

  //*AR: 190112: Read MC or data prediction file
...
*/
  ...
  EstHist = new TH1D("Pred", "Pred", EstHistTemp->GetNbinsX(), 0.5, EstHistTemp->GetNbinsX()+0.5);
  EstHistD = new TH1D("PredD", "PredD", EstHistDTemp->GetNbinsX(), 0.5, EstHistDTemp->GetNbinsX()+0.5);
  GenHist = new TH1D("Exp", "Exp", GenHistTemp->GetNbinsX(), 0.5, GenHistTemp->GetNbinsX()+0.5);
  GenHistD = new TH1D("ExpD", "ExpD", GenHistDTemp->GetNbinsX(), 0.5, GenHistDTemp->GetNbinsX()+0.5);
  ...
  if(doData){
    ...
    if(doClosurewoIsoTrackVeto){
      ...
      if(option.find("QCD")!=string::npos) sprintf(tempname,"Comparison_QCD_HDP_Full_Plot.pdf");
        else sprintf(tempname,"Comparison_Full_Plot_2016Recheck_190702.pdf");
