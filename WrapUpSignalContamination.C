...
void WrapUpSignalContamination(string type="T1bbbb",string outStr="Jul"){

  char tempname[200];
  char filenames[500];

  // open a file to write the output 
  sprintf(tempname,"%s_SignalFiles.root",type.c_str());
  ...
  // *AR 180622:Output files in TauHad2Multiple are already scaled to 1/pb
  ...
  sprintf(tempname,"InputFiles_Signal/V12_%s/%s_InputSignal.txt",type.c_str(),type.c_str());
...
// *AR -Jul22,2016-With Rishi's request to be consistent with Simon's naming
    ...
    temphist = (TH1D*) infile->Get("h_Prediction")->Clone();
