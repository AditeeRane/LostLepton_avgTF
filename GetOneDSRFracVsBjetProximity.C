...
void GetOneDSRFracVsBjetProximity(int hNum,char const * VarPre, char const * VarPreHadtaudRH, char const * VarPreHadtaudRL, char const * VarPreLLdRH,char const * VarPreLLdRL, char const * Sample,char const * TTbarLL,char const * LegHeader,double Legxmin,double Legymin,double Legxmax,double Legymax,char const *xRatioLabel,char const *yRatioLabel,bool logy, bool logx,int RatioNbins,double RatioXmin,double RatioXmax,double RatioYmin,double RatioYmax,double topMax){
  ...
  char hnamePre[500];
  char hnamePreHadtaudRH[500];
  char hnamePreHadtaudRL[500];
  char hnamePreLLdRH[500];
  char hnamePreLLdRL[500];
  char hnameRev[500];
  char tempWJet[500];
  char tempTTbar[500];
  char LabelX[500];
  ...
  //const * VarPre, char const * VarPreHadtaudRH, char const * VarPreHadtaudRL, char const * VarPreLLdRH,char const * VarPreLLdRL, char const * Sample,char const * TTbarSR

  sprintf(header,"%s",LegHeader);
  sprintf(hnamePre,"%s",VarPre);
  sprintf(hnamePreHadtaudRH,"%s",VarPreHadtaudRH);
  sprintf(hnamePreHadtaudRL,"%s",VarPreHadtaudRL);
  sprintf(hnamePreLLdRH,"%s",VarPreLLdRH);
  sprintf(hnamePreLLdRL,"%s",VarPreLLdRL);
...
*/
  ...
  TH1D *hTTbarPreHadtaudRH = (TH1D*)_fileTTbarLL->FindObjectAny(hnamePreHadtaudRH);
  TH1D *hTTbarPreHadtaudRL = (TH1D*)_fileTTbarLL->FindObjectAny(hnamePreHadtaudRL);
  TH1D *hTTbarPreLLdRH = (TH1D*)_fileTTbarLL->FindObjectAny(hnamePreLLdRH);
  TH1D *hTTbarPreLLdRL = (TH1D*)_fileTTbarLL->FindObjectAny(hnamePreLLdRL);
...
*/
  ...
  double LLdRLFrac,LLdRHFrac,HadtaudRLFrac,HadtaudRHFrac;
...
*/
  ...
  */
  ...
  TH1D *  cOne=(TH1D *) hTTbarPredRL->Clone("Ratio");
  cOne->Divide(hTTbarPre);
...
void GetOneDSRFracVsBjetProximity(){
