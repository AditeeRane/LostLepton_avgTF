...
void GetOneDTFTTbarPlusWjetPlots(int hNum,char const * VarCS, char const * VarPre, char const * VarPreLL, char const * VarPreHadtau,char const * Sample,char const * TTbarLL,char const * LegHeader,double Legxmin,double Legymin,double Legxmax,double Legymax,char const *xRatioLabel,char const *yRatioLabel,bool logy, bool logx,int RatioNbins,double RatioXmin,double RatioXmax,double RatioYmin,double RatioYmax,double topMax){
  ...
  //  hTTbarCS->SetLineWidth(2);

  //  hTTbarCS->SetFillColor(kRed+4);

  hTTbarPreLL->SetLineColor(kBlue);
  //  hTTbarPreLL->SetLineWidth(2);
  hTTbarPreLL->SetFillColor(kBlue);
  hTTbarPreLL->SetFillStyle(3013);

  hTTbarPreHadtau->SetLineColor(96);
  //  hTTbarPreHadtau->SetLineWidth(2);
  hTTbarPreHadtau->SetFillColor(96);
...
*/
  ...
  hTTbarCS->SetLabelSize(0.032);
  hTTbarCS->SetLabelFont(42);
  hTTbarCS->SetLabelSize(0.215,"XY");
  hExp->GetYaxis()->SetTitle(TopLabelY);
  hExp->GetYaxis()->SetTitleSize(0.05);
 
  //  hTTbarCS->GetYaxis()->SetTitleOffset(0.0);
  ...
  //  tl->SetNColumns(2);
  tl->AddEntry(hTTbarPreHadtau," SR:#tau_{h}","f");
  tl->AddEntry(hTTbarPreLL," SR:LL","f");
  tl->AddEntry(hTTbarCS," CR:1L","f");
...
void GetOneDTFTTbarPlusWjetPlots(){
  ...
  GetOneDTFTTbarPlusWjetPlots(1300,"h_CSStat_NJetvsNBtag_1D","h_Prediction_NJetvsNBtag_1D","h_PredictionLL_NJetvsNBtag_1D","h_PredictionHadtau_NJetvsNBtag_1D","TF_TTbarPlusWjet","Prediction_0_haddTTbar_0L_.root","t#bar{t}+Wjets",0.77,0.57,0.90,0.77,"bin index","Transfer Factor",1,0,19,1,20,0,2.49,200000);
